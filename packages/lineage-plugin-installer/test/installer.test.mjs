...
test("parseLineageVersion accepts JSON strings and CLI output", () => {
  assert.equal(parseLineageVersion('"0.1.4"'), "0.1.4");
  assert.equal(parseLineageVersion("lineage 0.1.4"), "0.1.4");
});
...
test("CLI preserves install --version as the Lineage compatibility selector", () => {
  ...
  assert.equal(output.pluginVersion, "0.1.4");
  assert.equal(output.lineageVersion, "0.1.4");
});
...
test("assertPluginManifest accepts exact package and plugin version match", () => {
  const manifest = {
    ...
    version: "0.1.4",
    lineage: { package: "@mean-weasel/lineage", version: "0.1.4" },
  };

  assert.equal(assertPluginManifest(manifest, "0.1.4"), manifest);
});
...
test("assertPluginManifest rejects plugin version mismatch", () => {
  assert.throws(
    ...
    /Plugin version 0\.1\.1 does not match 0\.1\.4/,
  );
});
...
test("assertPluginManifest rejects lineage compatibility mismatch", () => {
  assert.throws(
    ...
    /Plugin lineage version 0\.1\.1 does not match 0\.1\.4/,
  );
});
...
test("parseChecksumText accepts sha256 files with filenames", () => {
  ...
  assert.equal(parseChecksumText(`${hash}  lineage-codex-plugin-0.1.4.tgz\n`), hash);
...
test("releaseArtifactUrls derives GitHub release artifact URLs", () => {
  assert.deepEqual(releaseArtifactUrls({ version: "0.1.4" }), {
    artifactUrl:
      "https://github.com/mean-weasel/lineage/releases/download/v0.1.4/lineage-codex-plugin-0.1.4.tgz",
    checksumUrl:
      "https://github.com/mean-weasel/lineage/releases/download/v0.1.4/lineage-codex-plugin-0.1.4.tgz.sha256",
  });
});
...
test("installPluginDirectory copies a verified plugin to target directory", async () => {
  ...
  try {
    ...
    assert.equal(copied.version, "0.1.4");
  } finally {
    await rm(temp, { recursive: true, force: true });
  }
});
...
test("packPlugin dry-run validates artifact contents without writing dist files", async () => {
  ...
  assert.equal(result.dryRun, true);
  assert.equal(result.artifactName, "lineage-codex-plugin-0.1.4.tgz");
  ...
  await assert.rejects(readFile(path.resolve("dist-test-should-not-exist", "lineage-codex-plugin-0.1.4.tgz")));
});
...
test("installPluginArtifact verifies local release artifact and dry-runs install", async () => {
  ...
  try {
    ...
    assert.equal(result.dryRun, true);
    assert.equal(result.pluginVersion, "0.1.4");
    assert.equal(result.lineageVersion, "0.1.4");
...
test("installFromOptions can fetch a release artifact from resolved URLs", async () => {
  ...
  const urls = releaseArtifactUrls({
    version: "0.1.4",
    releaseBaseUrl: "https://example.test/releases/v0.1.4",
  });
...
test("installPluginArtifact rejects unsafe tarball entries", async () => {
  ...
  const artifactPath = path.join(temp, pluginArtifactFilename("0.1.4"));
