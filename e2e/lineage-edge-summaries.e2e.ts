...
test('shows quiet accessible edge summaries in every direction and toggles them canvas-wide', async ({ page, request }) => {
  ...
  try {
    ...
    await expect(page.locator(`.react-flow__edge[aria-label="${posterEdgeName}"]`)).toHaveCount(1);
